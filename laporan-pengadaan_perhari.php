...
<body class="A4 landscape">
  ...
  <section class="sheet padding-10mm" style="height: auto;font-size: 10px;">
    ...
    <div style="width: 100%;height: 2px;background-color: #3d3d3d;-webkit-print-color-adjust: exact;"></div>
      <h4 class="text-center">LAPORAN PENGADAAN PERHARI : <?= format_tanggal($_GET['tgl']);  ?></h4>
        <hr>
        ...
        <table class="table table-bordered"  id="example2">
          ...
          <tbody>
            <?php
              $datas = mysqli_query($koneksi, "select pengadaan.*, daftar_barang.nama, daftar_barang.satuan,permintaan.jumlah_minta,permintaan.total_permintaan,permintaan.tgl_permintaan,daftar_barang.harga_jual,  pelanggan.nama as nama_pel from pengadaan JOIN permintaan ON permintaan.id = pengadaan.permintaan_id JOIN daftar_barang ON daftar_barang.id = permintaan.barang_id JOIN pelanggan ON pelanggan.id = permintaan.pelanggan_id where pengadaan.tgl_pengadaan = '$tgl' group by pengadaan.id") or die(mysqli_error($koneksi));
